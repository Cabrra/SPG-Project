...
void EntityManager::UpdateAll( float elapsedTime )
{
	...
	m_bIterating = true;
	{
		...
		for( unsigned int bucket = 0; bucket < m_tEntities.size( ); bucket++ )
		{
			...
			for (unsigned int i = 0; i < vec.size(); i++)
 			{
				if (rect.IsPointInRectangle(vec[i]->GetRect().ComputeCenter()) || vec[i]->GetType() == Entity::ENT_CHALLENGE)
...
void EntityManager::RenderAll( void )
{
	...
	m_bIterating = true;
	{
		...
		for( unsigned int bucket = 0; bucket < m_tEntities.size( ); bucket++ )
		{
			...
			for (unsigned int i = 0; i < vec.size(); i++)
			{
				if (rect.IsPointInRectangle(vec[i]->GetRect().ComputeCenter()) /*|| vec[i]->GetType() == Entity::ENT_PUZZLE*/)
				vec[i]->Render();
			}
		}
	}
		...
		if( bucket1 != bucket2 )
		{
			...
			for( unsigned int i = 0; i < vec1.size(); i++ )
			{
				...
				for( unsigned int j = 0; j < vec2.size(); j++ )
				{
					...
					if (vec1[i] == vec2[j] || vec1[i]->GetIsDead() == true || vec2[j]->GetIsDead() == true)
						continue;
...
/**************************************************************/
// CheckCollisions
//	- check collision between the map tiles and a bucket of entities
void EntityManager::CheckCollisionWithTiles(Room* room, unsigned int bucket)
{
	if (bucket >= m_tEntities.size() || m_tEntities[bucket].size() == 0)
		return;
	EntityVector& vec = m_tEntities[bucket];

	for (size_t i = 0; i < vec.size(); i++)
		room->CheckCollision(vec[i]);
}

/**************************************************************/
// CheckCollisions
//	- check if there will be a collision between the entities
bool EntityManager::IsGoingToCollide(SGD::Point point, Entity* unit, unsigned int bucket)
{
	SGD::Rectangle rect = { point, unit->GetSize() };
	EntityVector& vec = m_tEntities[bucket];

	for (size_t i = 0; i < vec.size(); i++)
	{
		if (bucket == GamePlayState::BUCKET_PLAYER)
		{
			Player* plr = (Player*)vec[i];
			if (plr->GetSafeZone() == false || unit->GetType() == Entity::ENT_MOVABLE)
			{
				if (rect.IsIntersecting(vec[i]->GetRect()))
					return true;
			}
		}
		else
