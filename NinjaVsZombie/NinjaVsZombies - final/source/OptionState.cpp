...
/*virtual*/ void OptionState::Render(void)
{
	...
	pFont->Draw(out.str().c_str(), { 550, 150 }, 0.6f, SFXVol);

	LoadString(HInstance, IDS_MUSICVOLUME, buf, 255);
	pFont->Draw(buf, { 50, 200 }, 0.5F, MUSICVol);
	pFont->Draw(musicvol.str().c_str(), { 550, 200 }, 0.6f, MUSICVol);
	...
	pFont->Draw(buf, { 500, 350 }, 0.5f, OnFS);

	LoadString(HInstance, IDS_OFF, buf, 255);
	pFont->Draw(buf, { 575, 350 }, 0.5f, OffFS);
	...
	pFont->Draw(buf, { 50, 450 }, 0.5f, ExitOptions);
}