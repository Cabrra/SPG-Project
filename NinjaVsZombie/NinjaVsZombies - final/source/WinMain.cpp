...
#define WINDOW_CLASS_NAME	((const wchar_t*)L"Ninja VS Zombies")		// window class name
#define WINDOW_TITLE		((const wchar_t*)L"Ninja VS Zombies")		// window title
...
{	
	...
	if( pGame->Initialize( WINDOW_WIDTH, WINDOW_HEIGHT ) == false )
		return -3;
	pGame->GetInstance()->SetHInstance(hInstance);
...
HWND MakeWindow( HINSTANCE hInstance )
{
	...
	winClassEx.hIcon			= LoadIconW(hInstance, MAKEINTRESOURCEW(IDI_ICON1));
	winClassEx.hIconSm			= LoadIconW( hInstance, MAKEINTRESOURCEW(IDI_ICON1) );
